...
Claude Code 也有类似思想。`src/query/deps.py` 把 `callModel`、`microcompact`、`autocompact`、`uuid` 抽成依赖。生产环境用真实实现，测试可以注入 fake。这种依赖注入对 Agent 非常重要，因为模型调用昂贵、慢、不稳定，不适合每个测试都真调。
...
Claude Code 的 `src/query/deps.py` 体现了同样思想。它把生产依赖集中起来，方便测试替换。
...
```text
entrypoints/cli.python -> main.python -> REPL/QueryEngine -> query.py -> services/api/claude.py
```
