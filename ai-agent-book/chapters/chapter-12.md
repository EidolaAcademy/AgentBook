...
Claude Code 的 `src/query/deps.py` 就是同样思想。`query()` 不直接写死所有依赖，而是可以注入 `callModel`、`microcompact` 等。
...
engine = AgentEngine(:
    ...
    "tools": registry
    "cwd": process.cwd()
    "permissionPrompter": CliPermissionPrompter(rl)
```
...
Claude Code 的 `src/services/api/claude.py` 做的事情包括：
