...
6. Claude Code 的 `toolExecution.ts` 做了哪些产品级增强。
...
Claude Code 的 Tool 接口中也有 `validateInput()`，`toolExecution.ts` 会在权限判断前调用它。顺序很重要：如果输入本身无效，就没必要问权限。
...
async def run_hook(name: str, payload: dict[str, Any], timeout: float = 5.0) -> HookResult:
    try:
        ...
        return HookResult(True, f"hook 已执行: {name}")
    except asyncio.TimeoutError:
        return HookResult(False, f"hook 超时: {name}")
```
