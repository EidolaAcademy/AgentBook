...
async def ask_with_history(model: Any, messages: list[Message], user_text: str) -> Message:
...
def extract_tool_uses(message: AgentMessage) -> list[ToolUse]:
...
async def run_agent_loop(
    ...
    raise RuntimeError("Agent loop reached max_turns")
```
...
async def production_query_loop(
    model: Any,
    tools: dict[str, Any],
    messages: list[AgentMessage],
    ...
    for _ in range(config.max_turns):
        if config.enable_memory_prefetch:
            await prefetch_memory(messages)
        if config.enable_skill_discovery:
            await discover_relevant_skills(messages)

        messages = await compact_if_needed(messages, config.max_context_tokens)
        system_prompt = await build_system_prompt(messages, tools)
        assistant = await model.stream_complete(system_prompt, messages, tools)
        messages.append(assistant)
        ...
        tool_results = await execute_tools_with_permissions(tool_uses, tools)
        messages.extend(tool_results)

    raise RuntimeError("query loop stopped after max_turns")
```
