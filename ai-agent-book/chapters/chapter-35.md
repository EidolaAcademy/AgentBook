...
```python
def sanitizeAgentName(name: str):
    return name.replace(/@/g, '-')

def formatAgentId(name: str, teamName: str):
...
await sendMessage(:
    "to": 'researcher'
    "message": '请优先检查 token 刷新逻辑。'
    "summary": 'focus on refresh token'
```
...
await sendMessage(:
    "to": '*'
    "message": '大家停止修改文件，先汇报当前进度。'
    "summary": 'stop and report progress'
```
...
def assignTeammateColor(agentId: str):
    existing = assignments.get(agentId)
    if (existing) return existing

    color = COLORS[assignments.size % COLORS)]
    assignments.set(agentId, color)
    return color
```
...
await spawnTeam(:
    "teamName": 'auth-fix'
    "description": 'Fix authentication refresh bug'
    "leadAgentId": 'team-lead@auth-fix'
```
...
await spawnTeammate(:
    ...
    "prompt": "
    请只读检查认证刷新逻辑。
    重点找 refreshToken、session 更新、并发请求处理。
    不要修改文件。
    找到相关文件后用 SendMessage 汇报给 team-lead。
    "
```
...
await sendMessage(:
    "to": 'researcher'
    "summary": 'focus on token refresh'
    "message": '请优先检查 src/auth/token.py 和 src/auth/session.py。'
```
...
await sendMessage(:
    "to": '*'
    "summary": 'report current status'
    "message": '请停止新的探索，汇报当前发现和未完成事项。'
```
